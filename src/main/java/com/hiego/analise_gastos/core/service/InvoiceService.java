...
public class InvoiceService {
    ...
    public InvoiceService(InvoiceRepository repository, ChatModel chatModel) {
    ...
    public String analyzeInvoicesByMonth(String year, String month) {
        List<Invoice> invoices = findInvoicesByMonth(year, month);

        if (invoices.isEmpty()) {
            return "Nenhuma fatura encontrada para o mês " + month + "/" + year;
        }

        return analiseInvoiceWithAi(invoices);
    }


    private String analiseInvoiceWithAi(List<Invoice> invoices) {
        String invoicesData = prepareInvoicesData(invoices);
        double totalAmount = calculateTotalAmount(invoices);

        String prompt = String.format(
                "Analise as despesas do mês com base nas seguintes faturas:\n\n" +
                        "%s\n\n" +
                        "Total gasto no mês: R$ %.2f\n\n" +
                        "Com base nessas transações, faça uma análise detalhada e categorize os gastos em:\n" +
                        "1. Entretenimento\n" +
                        "2. Alimentação\n" +
                        "3. Estudos\n" +
                        "4. Saúde\n" +
                        ...
                        "Seja objetivo e use formatação clara.",
